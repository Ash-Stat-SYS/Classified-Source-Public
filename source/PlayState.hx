...
class PlayState extends MusicBeatState
{
	...
	public static var classifiedStoryMode:String = "";
	...
	public var camZoomingMultHUD:Float = 1;
	...
	public var laneUnderlayP1:FlxSpriteExtra;
	...
	var incrediblyInflatedHairySmellyBelly:Fuck;
	var camFuck:CamFuckShader;
	var uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD:HSLColorSwap;


    //l is real stage
	var fart:WateryShader;
	var bubbleEmitter:FlxEmitter;
	var SONICADVENTURE2BITCHHHHH:FlxEmitter;
	var nightSky:FlxSprite;
	var courtyard:FlxSprite;
	var clouds:FlxSprite;
	var trees:FlxSprite;
	var ripple:FlxSprite;
	var sky:FlxSprite;
	var wellF:FlxSprite;
	var backRock:FlxSprite;
	var flags:FlxSprite;
	var ship:FlxSprite;
	var overRockShip:FlxSprite;
	var walRocks:FlxSprite;
	var depths:FlxSprite;
	var foreRocks:FlxSprite;
	var filter:ShaderFilter;
	var poopbrah:FlxSprite;
	...
	// your copy fnf
	var ground:FlxSpriteExtra;
	var spotlight:FlxSpriteExtra;
	var stupidgaythreedfloor:FlxSpriteExtra;
	var stupidgaythreedspotlight:FlxSpriteExtra;
	var piracyisnouhhhhhhhhhhhhhhhhhhhh:FlxSpriteExtra;


	var wallBG:FlxSprite;
	var sillyRect:FlxRect;
	var shaderBlue = new BlueMaskShader();
	var thingyGroup:FlxSpriteGroup;

	// scuttlebug fnf
	var loogi:FlxSprite;
	var leantoad:FlxSprite;
	var grentoad:FlxSprite;
	var yeltoad:FlxSprite;
	var supaStar:Bool = false;

	var allowedToFocusShit:Bool = false;
	...
	override public function create()
	{
		...
		camGame = new FlxClippedCamera();
		camHUD = new FlxCamera();
		camOther = new FlxCamera();
		camHUD.bgColor.alpha = 0;
		camOther.bgColor.alpha = 0;

		wallBG = new FlxSprite();
		wallBG.scrollFactor.set();
		if(SONG.song.toLowerCase() == "better off")
			add(wallBG);
		...
		switch (curStage)
		{
			...
			case 'personalStage': //personalized stuff

			    GameOverSubstate.characterName = 'blank';
			    GameOverSubstate.loopSoundName = 'YourCopy-Game-over';
				GameOverSubstate.endSoundName = 'YourCopy-Retry';
				GameOverSubstate.uhhhhhhgayrights = true;

				piracyisnouhhhhhhhhhhhhhhhhhhhh = new FlxSpriteExtra();
				piracyisnouhhhhhhhhhhhhhhhhhhhh.loadGraphic(Paths.image("personalized/Piracy_is_a_one_piece"));
				piracyisnouhhhhhhhhhhhhhhhhhhhh.cameras = [camOther];
                piracyisnouhhhhhhhhhhhhhhhhhhhh.visible = false;
				piracyisnouhhhhhhhhhhhhhhhhhhhh.screenCenter();
				piracyisnouhhhhhhhhhhhhhhhhhhhh.scale.set(0.7, 0.7);
				add(piracyisnouhhhhhhhhhhhhhhhhhhhh);

				

				ground = new FlxSpriteExtra(-980, -540);
				ground.loadGraphic(Paths.image("personalized/floor"));
				add(ground);

				spotlight = new FlxSpriteExtra(-980, -540);
				spotlight.loadGraphic(Paths.image("personalized/spotlight"));
				spotlight.scrollFactor.set(0.8, 0.8);
			

				stupidgaythreedfloor = new FlxSpriteExtra(-680, -600);
				stupidgaythreedfloor.loadGraphic(Paths.image("personalized/ClassyEffect"));
				stupidgaythreedfloor.hide();
				stupidgaythreedfloor.scale.set(2, 2);
				add(stupidgaythreedfloor);

				stupidgaythreedspotlight = new FlxSpriteExtra(-420, -350);
				stupidgaythreedspotlight.loadGraphic(Paths.image("personalized/3DSpotlight"));
				stupidgaythreedspotlight.scrollFactor.set(0.3, 0.3);
				stupidgaythreedspotlight.hide();
			...
			case 'hallway': 
			        ...
			        GameOverSubstate.characterName = 'appbf-dead';
			        GameOverSubstate.loopSoundName = 'Wario-GameOver';
			        GameOverSubstate.endSoundName = 'Wario-Retry';
					gfGroup.visible = false;
			

			    

					hall = new FlxSprite(300, 230).loadGraphic(Paths.image("warioStage/Hallway"));
					hall.frames = Paths.getSparrowAtlas("warioStage/Hallway");
					hall.animation.addByPrefix("Hallway idle", "Hallway idle", 30, true);
					hall.animation.play("Hallway idle");
					hall.scale.set(1.4, 1.4);
					add(hall);

					wall = new FlxSprite(0, 135).loadGraphic(Paths.image("warioStage/thetitularhallway"));
					wall.alpha = 0;
					wall.scale.set(4,4);

					body = new FlxSprite(790, 750).loadGraphic(Paths.image("warioStage/running bf body"));
					body.frames = Paths.getSparrowAtlas("warioStage/running bf body");
					body.animation.addByPrefix("Bf idle body", "Bf idle body", 45, true);
					body.animation.play("Bf idle body");
					body.scale.set(0.65, 0.65);

					legs = new FlxSprite(790, 850).loadGraphic(Paths.image("warioStage/running bf shoe"));
					legs.frames = Paths.getSparrowAtlas("warioStage/running bf shoe");
					legs.animation.addByPrefix("Bf idle shoe", "Bf idle shoe", 45, true);
					legs.animation.play("Bf idle shoe");
					legs.scale.set(0.65, 0.65);

			case 'terminalStage': //i love scuttlebug !!!
			...
			GameOverSubstate.deathSoundName = 'speedrunDeath';
					...
					var clouds:BGSprite = new BGSprite('terminalStage/Background_Clouds', 200, -150, 0.6, 0.6);
					clouds.scrollFactor.set(0.6, 0.6);
					add(clouds);
				
					var clouds2:BGSprite = new BGSprite('terminalStage/Background_Clouds', -900, -450, 0.6, 0.6);
					clouds2.scrollFactor.set(0.6, 0.6);
					add(clouds2);
				
					var clouds3:BGSprite = new BGSprite('terminalStage/Background_Clouds', -1600, -50, 0.6, 0.6);
					clouds3.scrollFactor.set(0.6, 0.6);
					add(clouds3);
				
					var clouds4:BGSprite = new BGSprite('terminalStage/Background_Clouds', -2500, -50, 0.6, 0.6);
					clouds4.scrollFactor.set(0.6, 0.6);
					add(clouds4);
				
					var clouds5:BGSprite = new BGSprite('terminalStage/Background_Clouds', -3400, -15, 0.6, 0.6);
					clouds5.scrollFactor.set(0.6, 0.6);
					add(clouds5);
				
					var clouds6:BGSprite = new BGSprite('terminalStage/Background_Clouds', -4100, -45, 0.6, 0.6);
					clouds6.scrollFactor.set(0.6, 0.6);
					add(clouds6);
				
					var clouds7:BGSprite = new BGSprite('terminalStage/Background_Clouds', -5000, -50, 0.6, 0.6);
					clouds7.scrollFactor.set(0.6, 0.6);
					add(clouds7);
				
					var clouds8:BGSprite = new BGSprite('terminalStage/Background_Clouds', -6500, -50, 0.6, 0.6);
					clouds8.scrollFactor.set(0.6, 0.6);
					add(clouds8);
				
				
					var castle:BGSprite = new BGSprite('terminalStage/Castle', 1100, -400, 0.65, 0.65);
					add(castle);
				
					var bgground:BGSprite = new BGSprite('terminalStage/Background_BehindFloor', -950, 60, 0.75, 0.75);
					bgground.scrollFactor.set(0.75, 0.75);
					add(bgground);
				
					var bgground2:BGSprite = new BGSprite('terminalStage/Background_1BehindFloor', -950, 110, 0.75, 0.75);
					bgground2.scrollFactor.set(0.75, 0.75);
					add(bgground2);

					var hill1:FlxSprite = new FlxSprite(-400, -100);
					hill1.frames = Paths.getSparrowAtlas("terminalStage/Hill2_Bump");
					...
					hill2.frames = Paths.getSparrowAtlas("terminalStage/Hill1_Bump");
					...
					var ground:BGSprite = new BGSprite('terminalStage/Background_Floor', -950, 400, 1, 1);
					ground.scrollFactor.set(1, 1);
					add(ground);
				
					var regtoad:FlxSprite = new FlxSprite(675, 390);
					regtoad.frames = Paths.getSparrowAtlas("terminalStage/RedToad");
					...
					loogi.frames = Paths.getSparrowAtlas("terminalStage/Luigi");
					...
					yoshige.frames = Paths.getSparrowAtlas("terminalStage/yoshi_1");
					...
					blutoad.frames = Paths.getSparrowAtlas("terminalStage/BlueToad");
					...
					leantoad.frames = Paths.getSparrowAtlas("terminalStage/PurpleToad");
					...
					grentoad.frames = Paths.getSparrowAtlas("terminalStage/GreenToad");
					...
					yeltoad.frames = Paths.getSparrowAtlas("terminalStage/YellowToad");
			...
			case 'bobOmbBattlefield':
				GameOverSubstate.deathSoundName = 'mgameover';
				GameOverSubstate.characterName = 'blank';

				thingyGroup = new FlxSpriteGroup();
				add(thingyGroup);
				
				// lime.app.Application.current.window.minimized = true;
				var bg:BGSprite = new BGSprite('bobOmbBattlefield/bg', -610, -500, 0.6, 0.6);
				thingyGroup.add(bg);

				var hill:BGSprite = new BGSprite('bobOmbBattlefield/hill', -855, -750, 1.0, 1.0);
				hill.scale.set(1.2, 1.2);
				hill.updateHitbox();
				hill.antialiasing = true;
				thingyGroup.add(hill);

				sillyRect = new FlxRect(0, 0, 960, 540);
			...
			case 'void':
				GameOverSubstate.loopSoundName = 'GILBIERE_DIE';
				GameOverSubstate.endSoundName = 'GILQUEER_RESTARE';
				GameOverSubstate.characterName = 'door';
			...
			GameOverSubstate.loopSoundName = 'L-is-real-Gameover';
			GameOverSubstate.endSoundName = 'L-is-real-Retry';
		

				nightSky = new FlxSprite(-1020, -5050).loadGraphic(Paths.image("courtyard/Sky-Clouds-Stars"));
				add(nightSky);
			
				courtyard = new FlxSprite(-1020, -1880).loadGraphic(Paths.image("courtyard/well_mid"));
				// courtyard.alpha = 0.5;
				add(courtyard);
			
				clouds = new FlxSprite(-1020, -4000).loadGraphic(Paths.image("courtyard/clouds-fore"));
				add(clouds);
			
				wellF = new FlxSprite(-560, 735).loadGraphic(Paths.image("courtyard/well_fore"));

				ripple = new FlxSprite(-470, 820);
				ripple.frames = Paths.getSparrowAtlas("courtyard/waterwiggle");
				...
				sky = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/back"));
				sky.scrollFactor.set(0.4, 0.4);
				add(sky);

				backRock = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/backrocks"));
				backRock.scrollFactor.set(0.55, 0.55);
				add(backRock);

				flags = new FlxSprite(-1770, -425);
				flags.frames = Paths.getSparrowAtlas("courtyard/luigiunderwaterbg");
				flags.animation.addByPrefix("waters", "sail", 24, true);
				flags.animation.play("waters");
				flags.scrollFactor.set(0.55, 0.55);
				add(flags);

				ship = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/ship"));
				ship.scrollFactor.set(0.55, 0.55);
				add(ship);

				overRockShip = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/rocksovership"));
				overRockShip.scrollFactor.set(0.65, 0.65);
				add(overRockShip);

				walRocks = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/wallorocks"));
				walRocks.scrollFactor.set(0.8, 0.8);
				add(walRocks);

				depths = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/ground"));
				add(depths);

				foreRocks = new FlxSprite(-1350, -480).loadGraphic(Paths.image("courtyard/frontrocks"));
		...
		switch(curStage)
		{
			...
			// case 'terminalStage':
			// 	dad.shader = uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.shader;
			// 	boyfriend.shader = uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.shader;
				// trace("teetesahhsfdhdsdhsdshhsd");
		}
		...
		laneUnderlayP1 = new FlxSpriteExtra(0, 0).makeGraphic((112 * 4) + 2, FlxG.height);
		laneUnderlayP1.alpha = ClientPrefs.laneUnderlay;
		laneUnderlayP1.color = FlxColor.BLACK;
		laneUnderlayP1.scrollFactor.set();
		add(laneUnderlayP1);

		laneUnderlayP2 = new FlxSpriteExtra(0, 0).makeGraphic((112 * 4) + 2, FlxG.height);
		laneUnderlayP2.alpha = ClientPrefs.laneUnderlay;
		laneUnderlayP2.color = FlxColor.BLACK;
		laneUnderlayP2.scrollFactor.set();
	    if (curStage == 'hallway' || curStage == 'void')
		{
			laneUnderlayP2.visible = false;
			
			trace("ermm what happened to my opponent lane urppppp idk man !");
		}
		else
		{
			add(laneUnderlayP2);
		...
		timeTxt.setFormat(Paths.font("sm64-v2-1.ttf"), 28, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		scoreTxt.setFormat(Paths.font("sm64-v2-1.ttf"), 14, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	//	add(botplayTxt);
		...
		gameFlashLayer.cameras = [camOther];	
		...
		precacheList.set("SM64FileSelect", 'music');
	...
	{
		if(ClientPrefs.shaders != "Disabled") return new FlxRuntimeShader();
	...
	{
		if(ClientPrefs.shaders != "Disabled") return false;
	...
	public function startCountdown():Void
	{
		...
		if(curStage == "courtyard"){
			...
			introSoundsSuffix = "XDLOLLMFAOX3RAWR:3";
		}
		...
		switch(curStage)
		{
			case 'bobOmbBattlefield':
				...
				try
				{
					...
					allowedToFocusShit = true;
				}
				#end
				FlxTween.tween(blackScreen, {alpha: 0}, 3, {ease: FlxEase.sineInOut});
	...
	function eventPushed(event:EventNote) {
		switch(event.event) {
			case 'Watery':
				//this is preloading so it doesnt lag like dick
				if(ClientPrefs.shaders == "All")
					FlxG.game.setFilters([filter, MusicBeatState.filter, MusicBeatState.filter2]);
				else if(ClientPrefs.shaders == "Song-Specific")
	...
	override public function update(elapsed:Float)
	{
		...
		timeBar.createFilledBar(0x00870000, FlxColor.fromRGB(dad.healthColorArray[0], dad.healthColorArray[1], dad.healthColorArray[2]));
		timeBar.updateBar();
		...
		if(incrediblyInflatedHairySmellyBelly!=null)
			incrediblyInflatedHairySmellyBelly.iTime.value[0] = Conductor.songPosition / 1000;

		if(uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD != null){
			if(supaStar){
				if(uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue >= 1){
					uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue = 0;
				}
				else{
					uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue += elapsed * 1.25;
				}
			}else{
				if(uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue > 0){
					uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue -= elapsed * 1.25;
				}
			}
			// trace(uhmIdkFunnyPunAboutOverdosingOnDrugsLOLXD.hue);
		}
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			...
			case 'Water Transition':
				SONICADVENTURE2BITCHHHHH.emitting = true;
				new FlxTimer().start(1.5, function(tmr:FlxTimer) {
					SONICADVENTURE2BITCHHHHH.emitting = false;
				});
				switch(value1){
					case "1":
						FlxTween.tween(poopbrah, {alpha: 1}, 1.1);
					case "2":
						FlxTween.tween(poopbrah, {alpha: 0}, 0.7);
				}
			case 'Watery':
				fart = new WateryShader();
				if(ClientPrefs.shaders == "All")
				...
				trace("COCKCKCKCKC");
			case 'Your Copy Piracy Popup':
				piracyisnouhhhhhhhhhhhhhhhhhhhh.visible = true;
				...
				{
					case 0:
						FlxTween.tween(piracyisnouhhhhhhhhhhhhhhhhhhhh, {alpha: 1}, 0, {ease: FlxEase.quadInOut});
					case 1:
						FlxTween.tween(piracyisnouhhhhhhhhhhhhhhhhhhhh, {alpha: 0}, 0.7, {ease: FlxEase.quadInOut});
				}
			case 'Funhouse BF Run':
				switch(value1){
					case 'wall':
						wall.alpha = 1;
						FlxTween.tween(wall.scale, {x: 1, y: 1}, 0.5);
					case 'run':
						defaultCamZoom = 0.8;
						FlxTween.tween(boyfriendGroup, {x: 150}, 0.3, {ease: FlxEase.quadInOut});
				}
			case 'M Focus':
				// allowedToFocusShit = true;
				// lime.app.Application.current.window.borderless = true;
				// FlxTween.tween(camGame, {y: (FlxG.height / 2) - (camGame.height / 2)}, 5.2, {ease: FlxEase.quadInOut});				
					...
					case 'Camera Flash':
						...
						case 'Camera Zoom':
							...
							case 'Update Lyrics':
								...
								if(value1.trim()!=''){
									 lyrics = new FlxText(0, 540, 340, value1, 64);
									lyrics.cameras = [camOther];
									lyrics.setFormat(Paths.font("Mario64.ttf"), 48, FlxColor.fromString('#FFFFFF'), CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
	...
	public function endSong():Void
	{
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				else if(classifiedStoryMode == "house")
				{
					SONG = Song.loadFromJson("your-copy", "your-copy");
					LoadingState.loadAndSwitchState(new PlayState());
					classifiedStoryMode = "";
				}
				else if(classifiedStoryMode == "Grave")
				{
					SONG = Song.loadFromJson("funhouse", "funhouse");
					LoadingState.loadAndSwitchState(new PlayState());
					classifiedStoryMode = "house";
				}
				else
				{
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			var shitterCum:Map<String, Array<Int>> = [ 
				'singRIGHT' => [23, 0],
				'singUP' => [0, -23],
				'singDOWN' => [0, 20],
				'singLEFT' => [-20, 0],
		...
		if(curStage == 'bobOmbBattlefield'){
			var amntX = lime.app.Application.current.window.x + shitterCum.get(dad.animation.name)[0];
			var amntY = lime.app.Application.current.window.y + shitterCum.get(dad.animation.name)[1];
			var directionX:String = "";
			var directionY:String = "";

			if(amntX < lime.app.Application.current.window.x)
				directionX = "X-Left";
			if(amntX > lime.app.Application.current.window.x)
				directionX = "X-Right";
			if(amntY > lime.app.Application.current.window.y)
				directionY = "Y-Down";
			if(amntY < lime.app.Application.current.window.y)
				directionY = "Y-Up";
	...
	function goodNoteHit(note:Note):Void
	{
		if (!note.wasGoodHit)
		{
			...
			if(!note.noAnimation) {
				...
				else
				{
					...
					if (!note.isSustainNote
						...
						&& noteRows[note.gfNote ? 2 : note.mustPress ? 0 : 1][note.row].length > 1)
						{
							...
							if(note != animNote){
								boyfriend.playGhostAnim(chord.indexOf(note) - 1, animToPlay, true);
								// trace("mhm");
							}
				...
				if(curStage == 'bobOmbBattlefield'){
					// var amntX = lime.app.Application.current.window.x + shitterCum.get(boyfriend.animation.name)[0];
					// var amntY = lime.app.Application.current.window.y + shitterCum.get(boyfriend.animation.name)[1];
					// var directionX:String = "";
					// var directionY:String = "";
					...
					// 	FlxTween.tween(lime.app.Application.current.window, {x: canWindowMove(directionX, amntX) ? amntX : lime.app.Application.current.window.x, y: canWindowMove(directionY, amntY) ? amntY : lime.app.Application.current.window.y}, 0.3);
					// }
					if(incrediblyInflatedHairySmellyBelly.amount >= 0.03 && SONG.notes[curSection].mustHitSection){
						// trace("brah! kill da glitchez.");
						FlxTween.tween(incrediblyInflatedHairySmellyBelly, {amount: incrediblyInflatedHairySmellyBelly.amount / FlxG.random.int(2,4)});
					}else{
						// trace("dudes, dudettes, im thihnkin its too smal to doo anythangz.");
					}
				}
	...
	function canWindowMove(direction:String, amnt:Float){
		var returnThing:Bool = false;
		switch(direction){
			case "X-Left":
				if((lime.app.Application.current.window.x  + amnt) > 0)
					returnThing = true;
				else
					returnThing = false;
			case "X-Right":
				if((lime.app.Application.current.window.x + amnt) < (openfl.system.Capabilities.screenResolutionX - (FlxG.width / 2)))
					returnThing = true;
				else
					returnThing = false;
			case "Y-Up":
				if((lime.app.Application.current.window.y + amnt) > 0)
					returnThing = true;
				else
					returnThing = false;
			case "Y-Down":
				if((lime.app.Application.current.window.y + amnt) < (openfl.system.Capabilities.screenResolutionY - (FlxG.height / 2)))
					returnThing = true;
				else
					returnThing = false;
		}
	...
	override function destroy() {
		...
		if(SONG.song.toLowerCase() == "better-off" || SONG.song.toLowerCase() == "better off"){
			#if windows
			try
			{
				Sys.command('taskkill /IM \"Melt.exe\" /F');
				lime.app.Application.current.window.borderless = false;
				allowedToFocusShit = false;
			}
			#end
		}
		super.destroy();
	...
	override function beatHit()
	{
		...
		switch (curStage)
		{
			case 'courtyard':
				if(curBeat == 1){
					FlxTween.tween(camGame, {zoom: 0.55}, 6, {ease: FlxEase.sineInOut});
					FlxTween.tween(blackScreen, {alpha: 0}, 3, {ease: FlxEase.sineInOut});
				}
				case 'void':
					if(curBeat == 1){
			...
			case 'personalStage':
				if (curBeat == 808){
				stupidgaythreedfloor.show();
				stupidgaythreedspotlight.alpha = 0.5;
				stupidgaythreedspotlight.show();
				ground.hide();
				spotlight.hide();
				}
			...
			if (generatedMusic && !endingSong && !isCameraOnForcedPos)
				...
				if (camZooming && FlxG.camera.zoom < 1.35 && ClientPrefs.camZooms)
				{
					if(curStage != "bobOmbBattlefield"){
						...
						camHUD.zoom += 0.03 * camZoomingMultHUD;
					}
				}
		...
		for (i in 0...achievesToCheck.length) {
			...
			if(!Achievements.isAchievementUnlocked(achievementName) && !cpuControlled) {
				...
				switch(achievementName)
				{
					...
					case 'toastie':
						if(/*ClientPrefs.framerate <= 60 &&*/ ClientPrefs.shaders != "Disabled" && ClientPrefs.lowQuality && !ClientPrefs.globalAntialiasing) {
							unlock = true;
						}
					case 'debugger':
