...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		if (!initialized)
		{
			...
			// music.loadStream(Paths.music('classifelded_title'));
			// FlxG.sound.list.add(music);
			// music.play();

			if(FlxG.sound.music == null) {
				FlxG.sound.playMusic(Paths.music('classifelded_title'), 0);
			}
		}
		...
		pressenter.loadGraphic(Paths.image('pressenter'));
		pressenter.antialiasing = false;
		pressenter.scale.set(4.2, 4.2);
		add(pressenter);

		mariohead = new FlxSprite();
		mariohead.frames = Paths.getSparrowAtlas('MarioHead');
		...
		// mariohead.animation.play('startup');
	...
	function skipIntro():Void
	{
		if (!skippedIntro)
		{
			...
			mariohead.animation.finishCallback = function(name:String){
				mariohead.animation.play('idle');
			};
			skippedIntro = true;
		}
